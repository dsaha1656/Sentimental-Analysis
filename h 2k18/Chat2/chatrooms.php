<?php 
    ...
    if (checkVar($_SESSION['userid'])): 
        ...
        $getRooms = "SELECT *
        			 FROM chat_rooms";
        $roomResults = mysqli_query($dataBase['connection'],$getRooms);		  
                ...
                <ul>
                    <?php 
                        while($rooms = mysqli_fetch_array($roomResults)):
                            $room = $rooms['name'];
                            $query = mysqli_query($dataBase['connection'],"SELECT * FROM `chat_users_rooms` WHERE `room` = '$room' ") or die("Cannot find data". mysqli_error());
